...
if [ "$(echo "$TOPOLOGY_STATUS" | jq '.payload.value[].name' | cut -d'"' -f2 )" == "$GRAPH_TOPOLOGY_NAME" ]; then
...
MODULE_CONNECTION_STRING=$(az iot hub module-identity connection-string show --device-id "$DEVICE_NAME" --module-id AzurePerceptModule --hub-name "$IOTHUB_NAME" --key-type primary --query "connectionString" -o tsv)

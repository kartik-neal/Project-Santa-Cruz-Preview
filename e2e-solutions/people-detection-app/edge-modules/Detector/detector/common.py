...
def format_detectionslva(startX, startY, endX, endY, label_idx, confidence, timestamp):
  return {"bbox": [float(startX), float(startY), float(endX), float(endY)], "label": CLASSES[label_idx], "confidence": float(confidence), "timestamp": timestamp }

def format_detectionsopencv(startX, startY, endX, endY, label_idx, confidence):
